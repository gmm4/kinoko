...
public final class SkillInfo {
    ...
    public int getMpCon(User user, int slv) {
        ...
        final int[] mpConEquipSkillIDs = SkillConstants.getEquipSkillMpConChangeSkillIDs(skillId);
        for (int equipSkillId : mpConEquipSkillIDs){
            if(user.getSkillLevel(equipSkillId) > 0 && user.isEquipSkillActive(skillId, equipSkillId)){
                mpCon += user.getSkillStatValue(equipSkillId, SkillStat.x);
            }
        }
    ...
    private static SkillInfo fromStatic(int skillId, WzProperty skillProp) throws ProviderError {
        ...
        final Map<Integer, Integer> reqSkills = resolveReqSkills(skillProp);
        ...
        return new SkillInfo(
                ...
                Collections.unmodifiableMap(reqSkills),
    ...
    private static Map<Integer, Integer> resolveReqSkills(WzProperty skillProp) {
        final Map<Integer, Integer> reqSkills = new HashMap<>();
        if (skillProp.get("req") instanceof WzProperty reqProp) {
            for (var entry : reqProp.getItems().entrySet()) {
                reqSkills.put(WzProvider.getInteger(entry.getKey()), WzProvider.getInteger((entry.getValue())));
            }
        }
        return reqSkills;
    }
